public class VM {
	...
	public String run(boolean the_coffee_request_button, boolean the_coin_sensor, float TIME) {
        if ( ((TIME - the_request_timer) <= 30.0) && ((TIME - the_request_timer) >= 10.0) 
        		&& (the_system_mode == 3) ) { 
        	primed_the_coffee_machine_output = 1;
        	primed_the_system_mode = 1;
        } else
        if ( ((TIME - the_request_timer) <= 50.0) && ((TIME - the_request_timer) >= 30.0) 
        		&& (the_system_mode == 2) ) { 
        ...
        if ( (the_coffee_request_button == true && !(old_the_coffee_request_button == true))
        		&& (old_the_coin_sensor == false) && (the_coin_sensor == false)
        		&& ((TIME - the_request_timer) <= 30.0) && (the_system_mode == 0) ) {
        	primed_the_request_timer = TIME;
        ...
        return primed_the_system_mode
        	+ " " + primed_the_coffee_machine_output;
	}
