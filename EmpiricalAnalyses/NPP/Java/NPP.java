public class NPP {
    ...
    public String run(boolean the_blockage_button,
    		int the_water_pressure, boolean the_reset_button) {
    	
        if ( (the_water_pressure >= 9 && old_the_water_pressure < 9) && (the_pressure_mode == 1) ) {
        	primed_the_pressure_mode = 2;//permitted
    	} else
        if ( (the_water_pressure >= 10 && old_the_water_pressure < 10) && (the_pressure_mode == 2) ) {
        	primed_the_pressure_mode = 0;//high
		} else
        if ( (the_water_pressure < 9 && old_the_water_pressure >= 9) && (the_pressure_mode == 2) ) {
        	primed_the_pressure_mode = 1;//low
        } else
        if ( (the_water_pressure < 10 && old_the_water_pressure >= 10) && (the_pressure_mode == 0) ) {
        	primed_the_pressure_mode = 2;//permitted
        }
        
        if ( ((the_blockage_button == true) && !(old_the_blockage_button == true)) &&
        		(!(the_pressure_mode == 0)) && (!(the_reset_button == true))) {
        ...
        return primed_the_safety_injection_mode
        		+ " " + primed_the_overridden_mode
        		+ " " + primed_the_pressure_mode;
    }
